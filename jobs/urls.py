...
# URL patterns for the jobs app. Because jobs.urls is mounted at the root
# in the project urls.py, the homepage lives at / and everything else
# branches from there. The order matters — more specific patterns
# (jobs/saved/, jobs/post/) must come before the generic <int:pk> patterns
# so Django doesn't try to parse "saved" or "post" as a job ID.
...
urlpatterns = [
    ...
    # Individual job pages and actions.
    ...
    # Employer-side management pages.
    ...
    path('employer/applications/<int:app_pk>/cancel/', views.cancel_application, name='cancel_application'),
    ...
    # JSON search API endpoints — used by the search bar's autocomplete.
    ...
    # Extended application flow with CV upload support.
    ...
    # AJAX endpoints for the employer application management panel.
